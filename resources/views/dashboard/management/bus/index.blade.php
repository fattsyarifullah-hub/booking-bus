...
<div class="max-w-7xl mx-auto my-8 p-4">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach ($allBus as $item)
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col justify-between group hover:shadow-md transition-shadow duration-200">
                ...
                <div class="p-5 flex-1 flex flex-col justify-between">
                    ...
                    <div class="grid grid-cols-3 gap-2 pt-4 border-t border-slate-50 text-center">
                        <a href="{{ route('dashboard.management.bus.show', $item->id) }}" class="text-xs font-semibold text-violet-700 bg-violet-50 hover:bg-violet-100 py-2 rounded-lg transition-colors">
                            Detail
                        </a>
                        
                        <a href="{{ route('dashboard.management.bus.edit', $item->id) }}" class="text-xs font-semibold text-slate-600 bg-slate-50 hover:bg-slate-100 py-2 rounded-lg transition-colors">
                            Edit
                        </a>
                        ...
                        <form action="{{ route('dashboard.management.bus.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data bus ini?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="w-full text-xs font-semibold text-red-600 bg-red-50 hover:bg-red-100 py-2 rounded-lg transition-colors">
                                Hapus
                            </button>
                        </form>
                    </div>
